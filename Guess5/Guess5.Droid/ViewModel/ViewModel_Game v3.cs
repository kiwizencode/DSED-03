...
namespace Guess5.Droid.ViewModel
{
    public class ViewModel_Game : ReactiveObject
    {
        ...
        private void SetupObservable()
        {
            ...
            /* When the user highest score, update the Top Score chart, if posisible */
            this.WhenAnyValue(x => x.HighestScore)
                .Select(flag => (HighestScore != ""))
                .Subscribe((flag) => {
                    if(flag)
                    {
                        ...
                        /* Only store MAX_TOP_SCORES number of Score records in the database 
                           Always load MAX_TOP_SCORES number of Score records into a list
                         */
                        ...
                        int count = MAX_TOP_SCORES - score_chart.Count;
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            score.ID = ScoreRepository.SaveProfile(score);
                            score_chart.Add(score);
                        }

                        bool need_update = false;
                        for (int i = 0; i < score_chart.Count; i++)
                        {
                        ...
                        {
                            int i = 0;
                            foreach (var record in ScoreRepository.GetProfiles().OrderBy(x => x.ID))
                            {
                                ScoreModel data = score_chart[i];
                                record.Name = data.Name;
                                record.Score = data.Score;
                                ScoreRepository.SaveProfile(record);
                                i++;
                            }
                        }
                    }
