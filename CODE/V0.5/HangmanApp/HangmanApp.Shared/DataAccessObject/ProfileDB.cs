...
{
    public class ProfileDB : SQLiteConnection
    {
        ...
        public ProfileDB(string path) : base(path)
        {
            // create the tables
            CreateTable<Profile>();
        }
        ...
        /// <returns></returns>
        public IEnumerable<T> GetItems<T>() where T : IDataEntity, new()
        {
            lock (locker)
            {
        ...
        /// <returns></returns>
        public T GetItem<T>(int id) where T : IDataEntity, new()
        {
            lock (locker)
            {
        ...
        /// <returns></returns>
        public int SaveItem<T>(T item) where T : IDataEntity
        {
            lock (locker)
            {
                if (item.ID != 0)
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public int DeleteItem<T>(int id) where T : IDataEntity, new()
        {
            lock (locker)
            {
